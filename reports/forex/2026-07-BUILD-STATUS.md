...
| Azerion | — | — | — | Vendor xlsx into `data/sources/`; add 7.5% tech fee (internal only) |
| Google Ads | — | — | — | |
...
2. **Conversion maturity.** Newest July clicks were 4 days old at pull time; the conversion window is 90 days. Phase B conversions will likely rise — treat $237 as a ceiling that should improve. Re-pull before the deck is finalised. (June was re-verified live at 27/9/14 = 50, byte-identical to what the June report booked, so June is mature and the direction of the comparison is safe.)
3. **Legacy zero-conversion anomaly not explained.** policytest_v2 spent $4,039.90 at 100% Mexico with 7.19% CTR, produced 104 starts and zero submitted applications, after converting 27 times in June. The funnel data says drop-off rather than lag, but the cause is not established. Needs one focused look before it is characterised in client-facing material.
4. **Client tracker not reconciled.** Client-facing spend must match the client's own tracker. The July GGMI tracker has not been received. $10,624.94 is the platform figure.
