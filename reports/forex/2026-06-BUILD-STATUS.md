...
**2026-07-17: THE GOOGLE SLIDES FILE IS CANONICAL (19 slides) — do NOT re-upload a locally built PPTX over it.** Renzo added 2 market-volatility slides (positions 4-5: correlation tiles r=0.91 apps~gold / r=0.94 actives~VIX; Live Apps vs Budget + strategy). Reviewed and fixed in place via `gws slides presentations batchUpdate` replaceAllText (gws IS discovery-driven — slides/any Google API works even though `gws --help` lists only 5 services): corrected "Dec–Mar beat budget" (true only for downstream metrics; apps missed from Jan), "-57% volume" → "-78% from January peak", July tagged as forecast, "execution failure" reframed, "(internal)" dropped from source lines, bridge line added to Outcomes-vs-Plan tying volatility + organic-capture stories, all footers renumbered N/19. Correlation coefficients NOT independently verified (no clean VIX/GLD monthly source); gold-forecast range checked vs bank consensus (fits $4,400-6,300 band). Local pptx + Drive raw pptx (1yxhT…) are now EXPORTS of the canonical Slides. Compliance flag passed to Renzo: "gold dip-buying" creative angle needs client compliance review. Renzo sends when ready.
