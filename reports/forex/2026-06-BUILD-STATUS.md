...
- **PENDING FROM RENZO/VENDOR: (1) Azerion June GCG vendor file (apps/starts — completes S4/S14 + billable basis); (2) Native pilot detail (vendor, delivery, fee treatment); (3) optional: GCG Budget-vs-Actual dashboard screenshot for a vs-plan slide (GGMI parity).** When files land: update data workbook + rebuild deck S4/S14 via scripts (Slides not yet Renzo-edited, safe to re-upload IN PLACE until he starts editing — after that, gws slides batchUpdate only).
